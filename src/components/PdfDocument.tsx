...
const PdfDocument: React.FC<PdfDocumentProps> = ({ dateStates }) => {
  ...
  Object.entries(dateStates).forEach(([date, entry]) => {
    ...
    const transactions = [
      ...entry.cashIn.map(
        (item) => ({ ...item, type: "Cash-In" } as Transaction)
      ),
      ...entry.cashOut.map(
        (item) => ({ ...item, type: "Cash-Out" } as Transaction)
      ),
    ];

    transactions.forEach((transaction, index) => {
      if (count >= 14) {
        // Max 14 transaction rows per page to allow one summary row
          ...
          <View style={styles.tableContainer}>
            <View style={styles.table}>
              <View style={styles.tableRow}>
                ...
                <Text style={[styles.tableCellHeader, styles.tableCellWide]}>
                  (+) Amount
                </Text>
                <Text style={[styles.tableCellHeader, styles.tableCellWide]}>
                  (-) Amount
              ...
              {pageRows.map(({ date, transaction, isFirst }, index) => (
                ...
                >
                  ...
                  <Text style={styles.tableCell}>
                    {transaction.type !== "Summary" ? transaction.type : ""}
                  </Text>
                  <Text style={styles.tableCell}>
                    {transaction.source || ""}
                  </Text>
